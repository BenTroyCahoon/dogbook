...
//   const { fetchAllDogs } = useContext(DogContext);
...
//   const { id } = useParams();
  ...
  //     try {
  //       const dogs = await fetchAllDogs();
  //       const selectedDog = dogs.find((dog) => dog._id === id);
  //       if (selectedDog) {
  //         setDog(selectedDog);
  //         console.log("Selected Dog's Friends:", selectedDog.friends); // Logga vänner till den valda hunden
  //         setFriends(selectedDog.friends);
  //         // Set dog image if available
  //         setDogImage(selectedDog.image);
  //       } else {
...
//   useEffect(() => {
//     (async () => {
//       const dog = await fetchDogById(id); // Hämtar enskild hund med id
//       setDog(dog);
//     })();
//   }, [id]);
